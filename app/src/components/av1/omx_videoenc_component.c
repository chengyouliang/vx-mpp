...
#define FF_MIN_BUFFER_SIZE (4*1024)
...
OMX_ERRORTYPE omx_videoenc_component_Constructor(OMX_COMPONENTTYPE *openmaxStandComp,OMX_STRING cComponentName) {
  ...
  if(!strcmp(cComponentName, VIDEO_ENC_AV1_NAME)) {
  ...
  if(omx_videoenc_component_Private->video_encoding_type == OMX_VIDEO_CodingAV1) {
    omx_videoenc_component_Private->ports[OMX_BASE_FILTER_INPUTPORT_INDEX]->sPortParam.format.video.eCompressionFormat = OMX_VIDEO_CodingAV1;
  }
...
void SetInternalVideoEncParameters(OMX_COMPONENTTYPE *openmaxStandComp) {
  ...
  if (omx_videoenc_component_Private->video_encoding_type == OMX_VIDEO_CodingAV1) {
    strcpy(outPort->sPortParam.format.video.cMIMEType,"video/av1");
    outPort->sPortParam.format.video.eCompressionFormat = OMX_VIDEO_CodingAV1;
    outPort->sVideoParam.eCompressionFormat = OMX_VIDEO_CodingAV1;

    setHeader(&omx_videoenc_component_Private->pVideoav1, sizeof(OMX_VIDEO_PARAM_AV1TYPE));
  
    omx_videoenc_component_Private->pVideoav1.nPortIndex = 1;
    omx_videoenc_component_Private->pVideoav1.nSliceHeaderSpacing = 0;
  }
}
    ...
    case OMX_IndexParamStandardComponentRole:
      {
        ...
        if (!strcmp((char *)pComponentRole->cRole, VIDEO_ENC_AV1_ROLE)) {
          omx_videoenc_component_Private->video_encoding_type = OMX_VIDEO_CodingAV1;
        } else {
          return OMX_ErrorBadParameter;
        }
        SetInternalVideoEncParameters(openmaxStandComp);
