...
private:
    ...
    OMX_U32              misc[MPP_BUFFER_MODE_BUTT][MPP_BUFFER_TYPE_BUTT];
    OMX_U32              misc_count;
...
public:
    ...
    MppBufferGroupImpl  *get_group(const char *tag, const char *caller,
                                   ...
                                   OMX_U32 is_misc);
    OMX_U32              get_misc(MppBufferMode mode, MppBufferType type);
    void                put_group(const char *caller, MppBufferGroupImpl *group);
    MppBufferGroupImpl  *get_group_by_id(OMX_U32 id);
    void                dump(const char *info);
    OMX_U32              is_finalizing();
    void                inc_total(OMX_U32 size);
    void                dec_total(OMX_U32 size);
    OMX_U32              get_total_now() { return total_size; };
    OMX_U32              get_total_max() { return total_max; };
};
...
OMX_U32 mpp_buffer_debug = 0;

static MppBufLogs *buf_logs_init(OMX_U32 max_count)
{
...
static void buf_logs_write(MppBufLogs *logs, OMX_U32 group_id, OMX_S32 buffer_id,
                           MppBufOps ops, OMX_S32 ref_count, const char *caller)
{
    MppBufLog *log = NULL;
...
static MPP_RET put_buffer(MppBufferGroupImpl *group, MppBufferImpl *buffer,
                          OMX_U32 reuse, const char *caller)
{
    mpp_assert(group);
...
OMX_U32 mpp_buffer_to_addr(MppBuffer buffer, size_t offset)
{
    ...
    OMX_U32 addr = impl->info.fd + ((impl->offset + offset) << 10);
...
void MppBufferService::inc_total(OMX_U32 size)
{
    OMX_U32 total = __sync_add_and_fetch(&total_size, size);
    bool ret;

    do {
        OMX_U32 old_max = total_max;
        OMX_U32 new_max = MPP_MAX(total, old_max);

        ret = __sync_bool_compare_and_swap(&total_max, old_max, new_max);
    } while (!ret);
}

void MppBufferService::dec_total(OMX_U32 size)
{
    __sync_fetch_and_sub(&total_size, size);
}

OMX_U32 mpp_buffer_total_now()
{
    return MppBufferService::get_instance()->get_total_now();
}

OMX_U32 mpp_buffer_total_max()
{
      ...
      misc_count(0)
{
    ...
    for (i = 0; i < (OMX_S32)HASH_SIZE(mHashGroup); i++)
...
OMX_U32 MppBufferService::get_group_id()
{
    OMX_U32 id = 0;
    static OMX_U32 overflowed = 0;
...
OMX_U32 MppBufferService::get_misc(MppBufferMode mode, MppBufferType type)
{
    type = (MppBufferType)(type & MPP_BUFFER_TYPE_MASK);
    if (type == MPP_BUFFER_TYPE_NORMAL)
        return 0;
...
MppBufferGroupImpl *MppBufferService::get_group_by_id(OMX_U32 id)
{
...
OMX_U32 MppBufferService::is_finalizing()
{
    return finalizing;
}
