 ...
 */
static OMX_S32 find_dir_in_path(char *path, const char *dir_name,
                               size_t max_length)
{
    struct dirent **dir;
    OMX_S32 path_len = strnlen(path, max_length);
    OMX_S32 new_path_len = 0;
    OMX_S32 n;
...
static OMX_S32 check_sysfs_iommu()
{
    OMX_U32 i = 0;
    OMX_U32 dts_info_found = 0;
    OMX_U32 ion_info_found = 0;
    OMX_S32 ret = ION_DETECT_IOMMU_DISABLE;
    ...
    for (i = 0; i < MPP_ARRAY_ELEMS(dts_devices); i++) {
        ...
        if (find_dir_in_path(path, dts_devices[i], sizeof(path))) {
            if (find_dir_in_path(path, "iommu_enabled", sizeof(path))) {
                ...
                if (iommu_fp) {
                    OMX_U32 iommu_enabled = 0;
                    if (fread(&iommu_enabled, sizeof(OMX_U32), 1, iommu_fp))
...
static OMX_S32 ion_heap_id = -1;
static OMX_U32 ion_heap_mask = (1 << ION_HEAP_TYPE_SYSTEM);
