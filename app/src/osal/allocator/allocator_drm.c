...
static int drm_handle_to_fd(int fd, OMX_U32 handle, int *map_fd, OMX_U32 flags)
{
    int ret;
...
static int drm_fd_to_handle(int fd, int map_fd, OMX_U32 *handle, OMX_U32 flags)
{
    int ret;
    struct drm_prime_handle dph;
...
static int drm_alloc(int fd, size_t len, size_t align, OMX_U32 *handle, OMX_U32 flags)
{
    int ret;
...
static int drm_free(int fd, OMX_U32 handle)
{
...
static MPP_RET os_allocator_drm_alloc(void *ctx, MppBufferInfo *info)
{
    ...
    drm_dbg_func("dev %d get handle %d", p->drm_device,
                 (OMX_U32)((intptr_t)info->hnd));

    ret = drm_handle_to_fd(p->drm_device, (OMX_U32)((intptr_t)info->hnd),
...
static MPP_RET os_allocator_drm_import(void *ctx, MppBufferInfo *data)
{
    ...
    ret = drm_fd_to_handle(p->drm_device, data->fd, (OMX_U32 *)&data->hnd, 0);

    ret = drm_handle_to_fd(p->drm_device, (OMX_U32)((intptr_t)data->hnd),
...
static MPP_RET os_allocator_drm_free(void *ctx, MppBufferInfo *data)
{
    ...
    drm_fd_to_handle(p->drm_device, data->fd, (OMX_U32 *)&data->hnd, 0);
    ...
    return drm_free(p->drm_device, (OMX_U32)((intptr_t)data->hnd));
}
