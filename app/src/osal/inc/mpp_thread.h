...
public:
    ...
    {
    public:
        inline Autolock(Mutex* mutex, OMX_U32 enable = 1) :
            mEnabled(enable),
            mLock(*mutex) {
            if (mEnabled)
                mLock.lock();
...
public:
    ...
    Condition(int type);
    ~Condition();
    OMX_S32 wait(Mutex& mutex);
    OMX_S32 wait(Mutex* mutex);
    OMX_S32 timedwait(Mutex& mutex, OMX_S64 timeout);
    OMX_S32 timedwait(Mutex* mutex, OMX_S64 timeout);
    OMX_S32 signal();
    OMX_S32 broadcast();
...
inline OMX_S32 Condition::wait(Mutex& mutex)
{
...
inline OMX_S32 Condition::timedwait(Mutex& mutex, OMX_S64 timeout)
{
...
inline OMX_S32 Condition::signal()
{
    return pthread_cond_signal(&mCond);
}
inline OMX_S32 Condition::broadcast()
{
...
public:
    ...
    OMX_S32  wait(OMX_S64 timeout) { return mCondition.timedwait(mLock, timeout); }
