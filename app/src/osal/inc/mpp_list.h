...
public:
    ...
    OMX_S32 add_at_head(void *data, OMX_S32 size);
    OMX_S32 add_at_tail(void *data, OMX_S32 size);
    ...
    OMX_S32 del_at_head(void *data, OMX_S32 size);
    OMX_S32 del_at_tail(void *data, OMX_S32 size);

    // direct fifo operation
    OMX_S32 fifo_wr(void *data, OMX_S32 size);
    OMX_S32 fifo_rd(void *data, OMX_S32 *size);

    // for status check
    OMX_S32 list_is_empty();
    OMX_S32 list_size();

    // for vector implement - not implemented yet
    // adding function will return a key
    OMX_S32 add_by_key(void *data, OMX_S32 size, OMX_U32 *key);
    OMX_S32 del_by_key(void *data, OMX_S32 size, OMX_U32 key);
    OMX_S32 show_by_key(void *data, OMX_U32 key);
