...
private:
    ...
    OMX_U32  allocator_valid[MPP_BUFFER_TYPE_BUTT];
...
public:
    ...
    OMX_U32 get_allocator_valid(MppBufferType type);
};

OMX_U32 MppRuntimeService::get_allocator_valid(MppBufferType type)
{
...
MppRuntimeService::MppRuntimeService()
{
    ...
    if (allocator_valid[MPP_BUFFER_TYPE_ION] &&
        ...
        OMX_U32 path_len = MAX_DTS_PATH_LEN;
        OMX_U32 dts_path_len = snprintf(path, path_len, "%s", mpp_dts_base);
        char *p = path + dts_path_len;
        OMX_U32 allocator_found = 0;

        path_len -= dts_path_len;

        for (i = 0; i < MPP_ARRAY_ELEMS(mpp_vpu_names); i++) {
            for (j = 0; j < MPP_ARRAY_ELEMS(mpp_vpu_address); j++) {
                OMX_U32 dev_path_len = snprintf(p, path_len, "%s%s",
...
OMX_U32 mpp_rt_allcator_is_valid(MppBufferType type)
{
