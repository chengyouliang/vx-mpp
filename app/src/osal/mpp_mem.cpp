...
public:
     ...
     */
    OMX_S32  find_node(const char *caller, void *ptr, size_t *size, OMX_S32 *idx);
    /*
     */
    ...
    OMX_S32  chk_poison(MppMemNode *node);

    void    add_log(MppMemOps ops, const char *caller, void *ptr, void *ret,
                    size_t size_0, size_t size_1);

    void    dump(const char *caller);
    OMX_U32  total_now(void) { return m_total_size; }
    OMX_U32  total_max(void) { return m_total_max; }
...
static void show_mem(OMX_U32 *buf, OMX_S32 size)
{
...
{
    memset((OMX_U8 *)p - MEM_ALIGN, MEM_HEAD_MASK, MEM_ALIGN);
    memset((OMX_U8 *)p + size,      MEM_TAIL_MASK, MEM_ALIGN);
}
      ...
      m_total_max(0)
{
    ...
    if (debug & MEM_DEBUG_EN) {
        mpp_env_get_u32("mpp_mem_node_max", (OMX_U32 *)&nodes_max, MEM_NODE_MAX);
    ...
    if (debug & MEM_DEBUG_EN) {
        ...
        if (frees_cnt) {
            ...
            for (i = 0; i < frees_max; i++, node++) {
                if (node->index >= 0) {
                    os_free((OMX_U8 *)node->ptr - MEM_HEAD_ROOM(debug));
...
OMX_S32 MppMemService::find_node(const char *caller, void *ptr, size_t *size, OMX_S32 *idx)
{
    OMX_S32 i = 0;
    MppMemNode *node = nodes;
...
void *MppMemService::delay_del_node(const char *caller, void *ptr, size_t *size)
{
    ...
    *size = 0;
    ...
    if (frees_cnt >= frees_max) {
        ...
        OMX_S32 frees_last = frees_idx - frees_cnt;
        if (frees_last < 0)
...
OMX_S32 MppMemService::chk_poison(MppMemNode *node)
{
    if ((debug & MEM_POISON) == 0)
        return 0;

    // check oldest memory and free it
    OMX_U8 *node_ptr = (OMX_U8 *)node->ptr;
    OMX_S32 size = node->size;
    OMX_S32 i = 0;
    OMX_S32 start = -1;
    OMX_S32 end = -1;
...
void MppMemService::dump(const char *caller)
{
    ...
    OMX_S32 start = log_idx - log_cnt;
    OMX_S32 tmp_cnt = log_cnt;
...
void *mpp_osal_realloc(const char *caller, void *ptr, size_t size)
{
    ...
    size_t size_real = (debug & MEM_EXT_ROOM) ? (size_align + 2 * MEM_ALIGN) :
                       (size_align);
    void *ptr_real = (OMX_U8 *)ptr - MEM_HEAD_ROOM(debug);
...
OMX_U32 mpp_mem_total_now()
{
    AutoMutex auto_lock(&service.lock);
    return service.total_now();
}

OMX_U32 mpp_mem_total_max()
{
    AutoMutex auto_lock(&service.lock);
